  ...
  methods: {
    ...
    handleSelectionChange(range, oldRange) {
      if (!range && oldRange) this.$emit("blur", this.quill);
       ...
       $( "#quill-container .ql-editor p" ).unbind( "dblclick")
       $( "#quill-container .ql-editor p" ).bind( "dblclick",  function(e){
          e.preventDefault();  
          let tree = this
           $(this).find("~li").slideToggle()
          console.log('$(tree).next().prop("tagName")', $(tree).next().prop("tagName"))
          if($(tree).next().prop("tagName")=='UL' || $(tree).next().prop("tagName")=='LI'){
            $(tree).next().slideToggle(100)
          }
      });
